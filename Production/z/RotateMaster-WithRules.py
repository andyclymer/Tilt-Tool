...
def checkCurveSelfOverlap(pt0, pt1, pt2, pt3):
    ...
    p3 = MathPoint(*pt3)
    # Angles
    a1 = degrees(p0.angle(p1)) # first anchor to its bcp
    a2 = degrees(p3.angle(p2)) # second anchor to its bcp
    a3 = degrees(p0.angle(p2)) # anchor to anchor
    a4 = degrees(p3.angle(p1)) # anchor to anchor
    # Compare angles
    error = 0.6
    doOverlap = False
